"""Run a full NIAH matrix for the heterogeneous-layers ChronoQuant branch."""
import subprocess
import sys
import json
import time
...
for ctx in CONTEXTS:
    for depth in DEPTHS:
        ...
        print(f"[{i}/{total}] Heterogeneous ChronoQuant @ {label}", flush=True)
...
# Print summary table
print("\n" + "="*80)
print("NIAH RESULTS: Heterogeneous Layer-wise ChronoQuant (2-bit early / 4-bit late)")
print(f"Model: {MODEL}")
print("="*80)
...
# Save JSON
with open("/Users/Shared/AI/models/niah_heterogeneous_results.json", "w") as f:
    json.dump({"model": MODEL, "results": results, "generated_at": time.strftime("%Y-%m-%d %H:%M:%S")}, f, indent=2)
print(f"\nSaved to /Users/Shared/AI/models/niah_heterogeneous_results.json")
