...
class LearnableRotation:
    """Orthogonal rotation R that preserves attention."""
    
    def __init__(self, dim: int, n_planes: int = 32):
        self.dim = dim
        self.n_planes = n_planes
        self.planes = []  # list of (i, j, theta)
        
        # Initialize with random planes
        for _ in range(min(n_planes, dim // 2)):
            i, j = np.random.choice(dim, 2, replace=False)
            self.planes.append((i, j, mx.random.uniform(0, 2 * np.pi)))
    
    def forward(self) -> mx.array:
        """Compute R from current angles."""
        R = mx.eye(self.dim, dtype=mx.float32)
        for i, j, theta in self.planes:
            R = R @ givens_matrix(self.dim, i, j, theta)
        return R
    ...
    def compute_attention_loss(
        ...
        """Compute attention preservation loss."""
        R = self.forward()
        ...
        return mx.square(out_orig - out_rot).mean()
    
    def train_step(self, Q, K, V, lr: float = 0.01):
        """One training step - randomly perturb angles."""
        current_loss = self.compute_attention_loss(Q, K, V)
        ...
        for idx, (i, j, theta) in enumerate(self.planes):
            # Random perturb
            new_theta = theta + mx.random.uniform(-0.2, 0.2)
            
            # Apply and check
            old_theta = self.planes[idx][2]
            self.planes[idx] = (i, j, new_theta)
            
            new_loss = self.compute_attention_loss(Q, K, V)
            improvement = current_loss - new_loss
            
            if improvement > best_improvement:
                best_improvement = improvement
                best_new_theta = new_theta
                best_plane_idx = idx
            
            # Restore
            self.planes[idx] = (i, j, old_theta)
        
        # Apply best improvement
        if best_improvement > 0 and best_plane_idx is not None:
            i, j, _ = self.planes[best_plane_idx]
            self.planes[best_plane_idx] = (i, j, best_new_theta)
        
        return current_loss, best_improvement
    ...
    def train(
        ...
        for step in range(n_steps):
            loss, improvement = self.train_step(Q, K, V, lr)
...
class EndToEndKVCodec:
    ...
    def train(
        ...
        imp = mx.abs(K_rot).mean(axis=(0, 2))
        imp_np = np.array(imp)
...
def train_and_export(
    ...
    for layer_idx in kv_indices[:4]:  # First 4 layers
        ...
        # Sample shorter for speed
        Q = keys[:, :, :2, :]
        K = keys[:, :, :4, :]
        V = values[:, :, :4, :] if values is not None else K
